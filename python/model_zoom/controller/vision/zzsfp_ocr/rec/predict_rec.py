...
from controller.vision.zzsfp_ocr.rec.rec_decoder import CTCLabelDecode
...
class TextRecognizer(object):
    def __init__(self, rec_model_path=None, ort_providers=None):
        ...
        self.postprocess_op = CTCLabelDecode(character_dict=open(Util.app_path() + f"/models/zzsfp/ppocr_keys_v1.txt").read().splitlines(),
                                             character_type='ch', use_space_char=True)
        so = ort.SessionOptions()
        so.log_severity_level = 3
        sess = ort.InferenceSession(Util.app_path() + f"/models/zzsfp/{rec_model_file}", so, providers=ort_providers)
